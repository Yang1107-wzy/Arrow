...
本文档系统性地阐述 `arrow.py` 中箭杆摆动检测算法的数学基础，从图像坐标系变换到频域分析，提供严谨的数学证明和推导过程，并详细描述每个步骤的实现细节和代码对应关系。
...
**代码实现**：在 `arrow.py` 中，图像通过 OpenCV 的 `cv2.VideoCapture` 读取，获取的 `frame` 对象是一个三维数组（高度 × 宽度 × 通道数），其中通道顺序为 BGR（蓝-绿-红）。像素访问通过 `frame[y, x]` 进行，注意 OpenCV 使用 (行, 列) 顺序，即 y 坐标在前。
...
### 10.1 残差在圆检测中的作用
残差是评估圆拟合质量的关键指标。在 `arrow.py` 的算法框架中，残差从两个层面影响检测结果：
