...
def select_circle_roi(frame):
    ...
    cv2.putText(
        display_frame,
        "Drag mouse to select circular ROI, Enter to confirm, ESC to cancel",
        (10, 30),
...
def get_scale_reference(frame):
    ...
    cv2.putText(
        display_frame, instruction, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2
    )
    cv2.imshow("Set Scale Reference", display_frame)
    ...
    if points and len(points) == 2:
        ...
        while True:
            try:
                real_distance = float(
                    input("Enter the real distance between the two points (in mm, can be decimal): ")
                )
                if real_distance <= 0:
...
def detect_shaft_wobble(
    ...
    if len(x_offsets) > filter_order * 3:
        try:
            ...
            filtered_centers = np.column_stack(
                [stable_center[0] + filtered_x / scale_factor, stable_center[1] + filtered_y / scale_factor]
            )
    ...
    dom_freq, dom_amp = frequency_analysis(
        radial_offsets, fps, plot_title="Radial Offset Frequency Spectrum", show_plot=show_plots
    )
    ...
    print(
        f"Maximum distance between any two center points (filtered): {max_pairwise_distance:.4f} {units}"
    )
...
if __name__ == "__main__":
    video_files = [
        "/Volumes/Extreme SSD/直度/20251219直度视频/2516-01_C001H001S0001/2516-01_C001H001S0001.avi"
    ]
