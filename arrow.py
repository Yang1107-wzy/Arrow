...
def select_circle_roi(frame: np.ndarray) -> Tuple[Tuple[int, int], int]:
    ...
    while True:
        ...
        cv2.putText(
            temp,
            "Drag to set radius. Drag center to move. Enter=OK, R=reset, ESC=Cancel",
            (10, 30),
...
def get_scale_factor_mm_per_px(frame: np.ndarray) -> float:
    ...
    while True:
        ...
        cv2.putText(
            disp,
            "Click/drag 2 points. Enter=OK  R=reset  ESC=Cancel",
            (10, 30),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.7,
            (0, 0, 255),
    ...
    print(
        f"[Scale] pixel_distance={px_dist:.3f}px, real_distance={real_mm:.3f}mm -> scale={scale:.6f} mm/px"
    )
    return scale
...
def select_ignored_rectangles(
    frame_bgr: np.ndarray, edges: np.ndarray
...
def apply_rect_masks(
    binary_img: np.ndarray, rects: List[Tuple[int, int, int, int]]
) -> np.ndarray:
    out = binary_img.copy()
    for (x1, y1, x2, y2) in rects:
        out[y1:y2, x1:x2] = 0
...
def solve_center_voting_sse_fixed_radius(
    edges: np.ndarray,
    gray_for_grad: np.ndarray,
    fixed_radius_px: float,
    roi_box: Tuple[int, int, int, int],
    ring_inner_px: Optional[float] = None,
...
def fallback_hough_center(
    ...
    min_radius = max(int(round(ring_inner_px - 6)), 1)
    max_radius = max(int(round(ring_outer_px + 6)), min_radius + 1)
    circles = cv2.HoughCircles(
        masked_gray,
        cv2.HOUGH_GRADIENT,
        dp=1.2,
        minDist=max(int(round(ring_outer_px)), 20),
        param1=120,
        param2=20,
    ...
    for cx, cy, r in circles:
        ...
        radius_penalty = abs(float(r) - ring_outer_px)
        score = dist + 0.5 * radius_penalty
...
def bandpass_sos_zero_phase(
    x: np.ndarray, fs: float, low: float, high: float, order: int
) -> np.ndarray:
    nyq = 0.5 * fs
...
def main(show_plots: bool = True):
    ...
    while cap.isOpened():
        ...
        search_r = int(round(ring_outer_px + search_margin_px))
        ...
        cv2.circle(
            mask,
            (int(round(last_center[0])), int(round(last_center[1]))),
            search_r,
            255,
            -1,
        )
        ...
        if xs.size >= 40:
            ...
            ring = (d >= (ring_inner_px - ring_margin_px)) & (d <= (ring_outer_px + ring_margin_px))
            pts_ring = pts[ring]
        ...
        if sol is not None:
            ...
            fallback_center = fallback_hough_center(
                blur,
                mask,
                last_center,
                ring_inner_px=ring_inner_px,
        ...
        cv2.circle(frame, (cx, cy), int(round(ring_outer_px)), (0, 255, 0), 2)
        cv2.circle(frame, (cx, cy), int(round(ring_inner_px)), (0, 255, 0), 2)
